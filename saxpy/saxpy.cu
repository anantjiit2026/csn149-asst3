...
void saxpyCuda(int N, float alpha, float* xarray, float* yarray, float* resultarray) {
    ...
    double kernelEndTimer = CycleTimer::currentSeconds();

    printf("cuda kernel time: %f with badwitdth [%f GB/s]\n", kernelEndTimer-kernelStartTime,GBPerSec(totalBytes, kernelEndTimer-kernelStartTime));
    //
